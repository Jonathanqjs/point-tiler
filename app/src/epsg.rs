...
use pcd_exporter::cesiumtiles::make_tile_content;
use pcd_exporter::gltf::GlbOptions;
use pcd_exporter::tiling::{self, TileContent, TileTree, geometric_error};
use pcd_parser::parser::Extension;
...
use crate::{
    ConvertOptions, InputReference, check_and_get_extension, collect_file_sizes,
    estimate_processing_size, format_size, log_directory_summary, report_progress,
    should_use_in_memory,
};

pub(crate) enum CoordinateTransformer {
    Epsg(PointTransformer),
}

impl CoordinateTransformer {
    pub(crate) fn new(input: InputReference, output_epsg: u16) -> std::io::Result<Self> {
        match input {
    ...
    pub(crate) fn transform_points_in_place(
        &mut self,
        points: &mut [Point],
    ) -> std::io::Result<()> {
...
fn in_memory_workflow(
    ...
    report_progress(
        ...
        "Transforming CRS",
        Some(&format!(
            "Transforming {point_count} points to EPSG:{}",
            args.output_epsg
        )),
    );
    ...
    report_progress(args, 0.45, "Grouping points into tiles", None);
    ...
    report_progress(
        args,
        0.60,
        "Writing leaf tile files",
        Some(&format!("{} tiles", tile_map.len())),
    );
    tile_map
        .into_par_iter()
    ...
    let start_local = std::time::Instant::now();
    report_progress(args, 0.80, "Exporting 3D Tiles (GLB)", None);
    ...
    report_progress(args, 0.95, "Generating tileset.json", None);
    ...
    report_progress(args, 1.0, "3D Tiles ready", None);
    Ok(())
}
...
fn external_sort_workflow(
...

    {
        ...
        report_progress(args, 0.20, "Sharding and external sorting", None);
...

    {
        ...
        for (index, (shard, run_files)) in shard_runs.into_iter().enumerate() {
            ...
            let ratio = (index + 1) as f32 / total_shards.max(1) as f32;
            let p = 0.50 + ratio * 0.45;
            report_progress(
                args,
                p,
                "Processing shards",
        ...
        report_progress(args, 0.98, "Generating tileset metadata", None);
        let mut tree = TileTree::default();
